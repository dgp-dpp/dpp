...
export class MapaComponent implements OnInit {
  ...
  public unidadesCapacitacion: any[] = [];
  public marcador: any;
  ...
  ngOnInit(): void {
    this.Tuposicion();
    //this.AddUnidadesMedicas();
    //this.AddUnidadesCapacitacion();
  }
    ...
    this.mapabienesService.getUnidadesCapacitacionService().pipe(
      ...
      _data =>{
        _data = _data.map(_unidadesCapacitacion=>{
          const { id,unidad,direccion,mun,telefono,director,mun_atendi,pg,lng,lat,tipo_unida,costo_tota,urlPdf,icon }= _unidadesCapacitacion;
          return {
            id:id,
            unidad:unidad,
            direccion:direccion,
            ...
            telefono:telefono,
            director:director,
            mun_atendi:mun_atendi,
            position:{lat,lng},
            pg:pg,
            tipo_unida:tipo_unida,
            costo_tota:costo_tota,
            urlPdf:urlPdf,
            icon:icon
          }
        })
        console.log(_data);
        this.unidadesCapacitacion = _data;
      }
    )
